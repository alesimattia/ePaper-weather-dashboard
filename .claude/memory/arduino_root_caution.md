...
- Arduino concatena in una sola unità di compilazione **solo** i `.ino` nella root dello sketch, e ricorre soltanto in `<sketch>/src/`. Un secondo `.ino` nella root darebbe `setup()`/`loop()` duplicati e romperebbe la build: gli sketch di servizio vanno altrove. La sonda del pannello sta in `GxEPD2_SOLUM_ESL/examples/panel_diagnostic/panel_diagnostic.ino`, cioè nel submodule del driver, che il build del firmware ignora (`examples/` non è sotto `<sketch>/src/`) — vedi [[gxepd2_097c_driver]].
