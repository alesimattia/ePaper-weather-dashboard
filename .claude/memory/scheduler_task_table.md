...
**Conseguenza non ovvia della modalità `ESTERNA`: in finestra di manutenzione l'orologio non viene sincronizzato.** `ensureTimeSynced()` ha un solo chiamante, `wifiOn()`, che lì non viene mai invocata, e nessun altro avvia SNTP. I task di rete di quei minuti girano quindi con `time()` che conta l'uptime dal 1970: la fascia è fail-open e passano, la cadenza giornaliera resta sospesa, e le query dei calendari filtrano da `1970-01-01` ricevendo i **primi** eventi del calendario invece dei prossimi, con lo slot consumato per la cadenza piena. Il primo giro a finestra chiusa passa da `wifiOn()` e allinea tutto.
