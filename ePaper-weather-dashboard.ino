...
// converter -> convenzione bit=1=NOT color.
...
GxEPD2_3C<GxEPD2_097c_SOLUM_672x960, GxEPD2_097c_SOLUM_672x960::HEIGHT / 8> display(
  GxEPD2_097c_SOLUM_672x960(15, 27, 26, 25)
);
...
static GxEPDImage::Descriptor g_cinema_dynamic_desc = {
  GxEPDImage::FORMAT_BWRY_1BPP, CINEMA_W, CINEMA_H,
...
{
  free(g_cinema_black);  g_cinema_black  = nullptr;
  free(g_cinema_red);    g_cinema_red    = nullptr;
  free(g_cinema_yellow); g_cinema_yellow = nullptr;
...
{
  if (!g_cinema_attempted) return true;
...
static bool isActiveHour()
{
  ...
  struct tm t;
  localtime_r(&now, &t);
  return t.tm_hour >= WIFI_ACTIVE_HOUR_START
      && t.tm_hour <= WIFI_ACTIVE_HOUR_END;
}
...
void setup()
{
  ...
  /**
   * Sensore ambientale BME680 via I2C (Bosch BSEC2 in ULP, sample ogni
   * 5 min). Lo stato del calibratore, se presente in NVS, viene
   * ripristinato qui dentro.
   */
  Indoor::begin();
  /**
...
void loop()
{
  if (Ota::windowOpen())
  {
    ...
    if (WiFi.status() == WL_CONNECTED)
    {
       ...
       * cosi' il display viene disegnato non appena il meteo e' sceso, anche
       * se la chiamata One Call e' fallita (placeholder "--" per i dati
       * mancanti). No-op dopo il primo refresh.
       */
